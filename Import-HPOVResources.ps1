...
Function Import-LogicalEnclosure([string]$sheetName, [string]$WorkBook, [string]$subdir)
{
	...
	foreach ( $le in $List)
	{
		...
		if ($enclosureName)
		{
			...
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'Set-HPOVEnclosure -inputObject $this_enclosure -Name $enclosureNames[$i]' -isVar $False -indentlevel 3))	
		...
		newLine
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('New-HPOVLogicalEnclosure -Name "{0}" {1}{2}{3}' -f $name, $enclParam, $egParam, $fwParam ) -isVar $false -indent 1) )
