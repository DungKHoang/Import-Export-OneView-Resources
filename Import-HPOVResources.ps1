...
function generate-scopeCode( $scopes, $indentlevel = 0)
{
	...
	$scopeCode	= ($TAB * $indentLevel) + ('{0}' -f $scopeList)  + ' | % { get-HPOVScope -name $_ | Add-HPOVResourceToScope -InputObject $object }'
...
function connect-Composer([string]$sheetName, [string]$WorkBook, $PSscriptCode )
{
	...
	[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix '$d = disconnect-HPOVMgmt -ApplianceConnection $global:ConnectedSessions' -isVar $false -indentLevel 1))
	endIfBlock 
	generate-credentialCode -username $userName -password $password -component 'OneView' -PSscriptCode $PSscriptCode
	[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('connect-HPOVMgmt -hostname {0} -credential $cred -loginAcknowledge:$True -AuthLoginDomain "{1}" ' -f $hostName,$authDomain) -isVar $False ))
    newLine
...
Function Import-firmwareBaseline([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
    ...
    foreach ($fw in $List)
    {
		...
		if ($filename)
		{
			...
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('Add-HPOVBaseline -file  "{0}"' -f $filename) -isVar $False ))
            newLine
        }
	}
	...
	if ($List)
	{
		[void]$PSscriptCode.Add('Disconnect-HPOVMgmt')
	}
	...
	foreach ($dc in $List)
	{
		...
		if ($name  -or $width -or $depth )
		{
			...
			if ($primaryContact)
			{
				...
				$value					= 'Get-HPOVRemoteSupportContact -Name "{0}" ' -f $primaryContact
			...
			if ($secondaryContact)
			{
				...
				$value					= 'Get-HPOVRemoteSupportContact -Name "{0}" ' -f $secondaryContact
			...
			$prefix		= 'New-HPOVDatacenter {0}{1}{2}{3} `' -f $nameParam,$widthParam,$depthParam,$millimetersParam
...
Function Import-rack([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ($rack in $List)
	{
		...
		$prefix 			= 'Add-HPOVRackToDataCenter {0}{1}{2} ' 	-f $inputParam,$dcParam, $coordParam
...
Function Import-proxy([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ($proxy in $List)
	{
		...
		# Code
		# ensure that there is no space after backstick(`)
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('Set-HPOVApplianceProxy {0}{1}{2}{3}' -f $hostNameParam,$portParam,$httpsParam,$userParam) -isVar $False ))
		newLine
...
Function Import-TimeLocale([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ($time in $List)
	{
		...
		# Code
		# ensure that there is no space after backstick(`)
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('Set-HPOVApplianceDateTime -locale "{0}" -ntpServers {1} {2} {3}' -f $locale,$ntpServers,$syncParam,$pollParam) -isVar $False ))
		newLine
 ...
 Function Import-backup([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
 {
	 ...
	 foreach ($bkp in $List)
	 {
		...
		if ($enabled -like 'True')
		{
			...
			# Code
			# ensure that there is no space after backstick(`)
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('Set-HPOVAutomaticBackupConfig {0} `' -f $remoteParam) -isVar $False ))
		...
		{
			# Code
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'Set-HPOVAutomaticBackupConfig  -Disabled:$True '  -isVar $False ))
		}
 ...
 Function Import-repository([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
 {
	 ...
	 foreach ($repo in $List)
	 {
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('New-HPOVExternalRepository -Name "{0}" {1}{2}{3}{4}{5}' -f $name, $hostParam, $httpParam, $dirParam, $credParam, $certParam) -isVar $False ))
	 ...
	 foreach ($scope in $List) 
	 {
		...
		if ($resource)
		{
			...
			foreach ($res in $ResArray)
			{
				...
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('get-HPOVScope -name "{0}" | Add-HPOVResourceToScope -InputObject $resources ' -f $name) -isVar $False -indentlevel 2))
				endIfBlock -indentlevel 1
				newLine
			}
		...
		endIfBlock		-condition '$null -eq (get-HPOVScope....'
		# Skip creating because resource already exists
 ...
 Function create-snmpTrap($List, $isSnmpAppliance=$True, $indent=0 )
 {
	...
	$isSnmpLig 			= -not $isSnmpAppliance
	 ...
	 foreach ($trap in $List) 
	 {
		...
		$trapCmd 		= if ($isSnmpAppliance) { 'New-HPOVApplianceTrapDestination '} else {'New-HPOVSnmpTrapDestination '}
 ...
 function create-snmpV3User($List, $isSnmpAppliance=$True, $indent=0)
 {
     ...
     foreach ($snmp in $List) 
	 {
		...
		if ($snmpV3User)
		{
			...
			[void]$PSscriptCode.Add(( Generate-PSCustomVarCode -Prefix ('{0}' -f $new_snmpv3user) -value ('new-HPOVSnmpV3User {0} -UserName "{1}" {2}' -f $applianceSnmpParam, $snmpV3User, $secParam ) -isVar $false -indentlevel  $indent ))
 ...
 Function Import-YMLSNMP([string]$sheetName, [string]$WorkBook, [string]$YMLfiles )
{
	...
	{	
		...
		if ($communityString)
		{
			...
			[void]$YMLscriptCode.Add((Generate-ymlTask 		 	-title $title -comment $comment -HPOVTask 'oneview_appliance_device_read_community'))
		...
		foreach ($_user in $snmpV3UserList)
		{
			...
			[void]$YMLscriptCode.Add((Generate-ymlTask 		 		-title $title -comment $comment -HPOVTask 'oneview_appliance_device_snmp_v3_users'))
		...
		foreach ($_trap in $snmpTrapList)
		{
			...
			if ($snmpV3User)
			{
				...
				[void]$YMLscriptCode.Add((Generate-ymlTask 		 		-title $title -comment $comment -HPOVTask $ovTask -isData $False))
			...
			[void]$YMLscriptCode.Add((Generate-ymlTask 		 		-title $title  -HPOVTask $ovTask))
 ...
 Function Import-remoteSupport([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
 {
	 ...
	 if ($List)
	 {
		 ...
		 if ($enable -eq 'True')
		 {
			...
			[void]$PSscriptCode.Add( (Generate-PSCustomVarCode -prefix ('Set-HPOVRemoteSupport -enable -CompanyName "{0}" {1} {2}' -f $companyName, $credParam, $optParam ) ))
		 }
		 else 
		 {
			[void]$PSscriptCode.Add( (Generate-PSCustomVarCode -prefix 'Set-HPOVRemoteSupport -disable ' ))
		 }
 ...
 Function Import-ligsnmp([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
 {
	 ...
	 foreach ($snmp in $List) 
	 {
		...
		if ($communityString)
		{
			...
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('Set-HPOVSnmpReadCommunity -name "{0} "' -f $readCommunity)  -isVar $False ))
			newLine
		}
 ...
 Function Import-user([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
 {
	 ...
	 foreach ($rs in $rsList)
	 {
		 ...
		 ifBlock 		-condition ('if ($null -eq (Get-HPOVRemoteSupportContact | where email -eq "{0}" ))' -f $email)
		 [void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('new-HPOVRemoteSupportContact {0}{1}{2}{3}' -f $defaultParam, $nameParam,$languageParam, $notesParam) -isVar $False -indentlevle 1))		 
		 endIfBlock
	 ...
	 foreach ($user in $ovList)
	 {
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('New-HPOVUser -username "{0}" -password "{1}" `' -f $name, $password) -isVar $False -indentlevel 2))
		...
		endIfBlock		-condition 'Get-HPOVUser...'
 ...
 Function Import-addressPool([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
 {
	 ...
	 foreach ($addr in $List)
	 {
		...
		if ($poolType -like "ip*")
		{
			...
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'subnet' -Value (" Get-HPOVAddressPoolSubnet | where networkId -eq '{0}'" -f $networkId ) -indentlevel 1) )

			ifBlock		-condition 'if ($subnet -ne $null) ' -indentlevel 1
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'addressPool' -Value 'Get-HPOVAddressPoolRange | where subnetUri -match ($subnet.uri)' -indentlevel 2) )
			...
			$value 			= 'new-HPOVAddressPoolSubnet -NetworkId $networkId -SubnetMask $subnetMask ' + $gwParam + $dnsParam + $domainParam
		...
		{
			$value = 'Get-HPOVAddressPoolRange| where {($_.name -eq $poolType) -and ($_.rangeCategory -eq $rangeType)} '
			...
			$value = 'Get-HPOVAddressPoolRange| where {($_.name -eq $poolType) -and ($_.rangeCategory -eq "Generated")} '
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'poolToDelete' -Value $value ) )
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix '$poolToDelete |  Remove-HPOVAddressPoolRange -confirm:$False' -isVar $False ) )
		...
		if ($poolType -like "ip*")
		{
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('New-HPOVAddressPoolRange -IPSubnet $subnet -name "{0}" {1}' -f $name,  $addressParam ) -isVar $False -indentlevel 1))
		}
		else 
		{
			...
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('New-HPOVAddressPoolRange -PoolType $poolType -RangeType $rangeType {0} ' -f  $addressParam ) -isVar $False -indentlevel 1))	
		}
		endIfBlock
 ...
 Function Import-YMLaddressPool_ipv4([string]$sheetName, [string]$WorkBook, [string]$YMLfiles )
{
	 ...
	 foreach ($addr in $List)
	 {
		...
		[void]$YMLscriptCode.Add((Generate-ymlTask 			-title $title -comment $comment -HPOVTask 'oneview_id_pools_ipv4_subnet'))
		...
		[void]$YMLscriptCode.Add((Generate-ymlTask 			-title $title  -HPOVTask 'oneview_id_pools_ipv4_range'))
 ...
 Function Import-ethernetNetwork([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
 {
     ...
     foreach ($net in $List)
     {
         ...
         if ($subnetID)
         {
             ...
             [void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'ipV4subnet' -Value ("Get-HPOVAddressPoolSubnet -NetworkID `'{0}`'" -f $subnetID ) -indentlevel 1))
         ...
         if ($ipV6subnetID)
         {
             ...
             [void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'ipV6subnet' -Value ("Get-HPOVAddressPoolSubnet -NetworkID `'{0}`'" -f $ipV6subnetID ) -indentlevel 1))
         ...
         [void]$PSscriptCode.Add(( Generate-PSCustomVarCode -Prefix 'New-HPOVNetwork -Name $name  -PrivateNetwork $PLAN -SmartLink $smartLink -VLANType $VLANType  -purpose $purpose `' -isVar $False -indentLevel 1)) 
         ...
         {
             newLine
             [void]$PSscriptCode.Add( (Generate-PSCustomVarCode -Prefix 'object' -Value 'get-HPOVNetwork | where name -eq $name' -indentlevel 1))
 ...
 Function Import-YMLethernetNetwork([string]$sheetName, [string]$WorkBook, [string]$YMLfiles )
{
     ...
     foreach ($net in $List)
     {
		 ...
		 [void]$YMLscriptCode.Add((Generate-ymlTask 		 -title $title  -HPOVTask 'oneview_ethernet_network'))	
...
Function Import-fcNetwork([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
    ...
    foreach ($net in $List)
    {
		...
		if ($managedSan)
		{
			...
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'managedSAN' -Value ('Get-HPOVManagedSAN -Name $SANname') -indentlevel 1))		
		}
		...
		newLine
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('New-HPOVNetwork -Name $name -Type $Type `') -isVar $False -indentlevel 1) )
...
Function Import-YMLfcNetwork([string]$sheetName, [string]$WorkBook, [string]$YMLfiles )
{
     ...
     foreach ($net in $List)
     {
		...
		if ($vlanId) # fcoe network
		{
			...
			[void]$YMLscriptCode.Add((Generate-ymlTask 			-title $title -comment $comment -HPOVTask 'oneview_fcoe_network'))
			[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix 		name					-value $name					-indentlevel $indentDataStart ))	
			[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix 		vlanId					-value $vlanId					-indentlevel $indentDataStart ))		
		}
		else # fc network
		{					
			...
			[void]$YMLscriptCode.Add((Generate-ymlTask 			-title $title -comment $comment -HPOVTask 'oneview_fc_network'))				
			[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix 		type					-value $_category				-indentlevel $indentDataStart ))
			[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix 		name					-value $name					-indentlevel $indentDataStart ))
...
Function Import-NetworkSet([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
    ...
    foreach ($ns in $List)
	{
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'net' -Value ("get-HPOVNetworkSet -name '{0}' -ErrorAction SilentlyContinue" -f $name) )) #HKD
		...
		if ( $networks )
		{
			...
			$value 		= ('{0}' -f $netList ) + ' | % { get-HPOVNetwork -name $_ }'
		...
		newLine
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('New-HPOVNetworkSet -Name $name{0}{1}{2}{3}' -f $pBWparam, $mBWparam, $netParam, $untaggedParam) -isVar $False -indentlevel 1))		
...
Function Import-YMLnetworkSet([string]$sheetName, [string]$WorkBook, [string]$YMLfiles )
{
     ...
     foreach ($ns in $List)
     {
		...
		$scopes             = if ($ns.scopes) {$ns.scopes.Split('|')} else {$Null}
		 ...
		 [void]$YMLscriptCode.Add((Generate-ymlTask 		-title $title -comment $comment -HPOVTask 'oneview_network_set'))	
...
Function Import-LogicalInterconnectGroup([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ($L in $list)    
	{
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'lig' 						-Value ("get-HPOVLogicalInterconnectGroup -name '{0}' -ErrorAction SilentlyContinue" -f $name) )) #HKD
		...
		if ($isNotSAS)
		{
			...
			if ($internalNetworks)
			{
				...
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'internalNetworks'			-Value ('{0}' -f $networks + ' | % {Get-HPOVNetwork -name $_}' 	) -indentlevel 1))
			...
			if ($snmpConfigList) 		# Have snmpConfiguration for this lig?
			{
				...
				{	
					[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'snmpConfiguration' -value ('New-HPOVSnmpConfiguration {0}{1}{2}{3}{4} ' -f $communityStringParam, $contactParam, $accessListParam, $snmpV3userParam, $snmpTrapDestinationParam)  -indentlevel 1))
					newLine
			...
			$ligVariable    = '$lig'
			newLine
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('{0} = New-HPOVLogicalInterconnectGroup -Name $name {1}{2}{3} `' -f $LigVariable, $fabricModuleParam, $FrameCountParam , $ICBaySetParam) -isVar $false -indentlevel 1))
		...
		{
			$ligVariable    = '$lig'
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('{0} = New-HPOVLogicalInterconnectGroup -Name $name {1}{2}{3}{4}' -f $LigVariable, $fabricModuleParam, $FrameCountParam , $ICBaySetParam, $baysParam) -isVar $false -indentlevel 1))
...
Function Import-UplinkSet([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ( $upl in $List)
	{
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'lig' 						-Value ("get-HPOVLogicalInterconnectGroup -name '{0}' -ErrorAction SilentlyContinue" -f $ligName ) ))	#HKD	
		...
		if ($networkType -ne 'FibreChannel') 
		{
			...
			if ($networkSets)
			{
				...
				$netList    = '@({0})' -f [string]::Join($Comma, $arr)
				$value 		= ('{0}' -f $netList ) + ' | % { get-HPOVNetworkSet -name $_ }'
			...
			if ($nativeNetwork)
			{
				...
				$value 			= "get-HPOVNetwork -name '{0}' " -f $nativeNetwork
		...
		newLine
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -prefix ('New-HPOVUplinkSet -InputObject $lig -Name $name -Type $networkType `') -isVar $False -indentlevel 1)) 
...
Function Import-YMLUplinkSet([string]$sheetName, [string]$WorkBook, [string]$YMLfiles )
{
	...
	foreach ( $upl in $List)
	{
		...
		[void]$YMLscriptCode.Add((Generate-ymlTask 		 -title $title -HPOVTask 'oneview_logical_interconnect_group'))
...
Function Import-YMLligSNMP([string]$sheetName, [string]$WorkBook, [string]$YMLfiles )
{
	...
	{	
		...
		[void]$YMLscriptCode.Add((Generate-ymlTask 		 -title $title -comment $comment -HPOVTask 'oneview_logical_interconnect_group'))
		[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix 		'name'				-value $ligName					-indentlevel $indentDataStart ))
...
Function Import-LogicalSwitchGroup([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ( $swg in $List)
	{
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('new-HPOVLogicalSwitchGroup -name "{0}" -switchType $switchType -NumberOfSwitches {1}' -f $name,$numberofSwitches) -isVar $False -indentlevel 1))
...
Function Import-LogicalSwitch([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ( $lsw in $List)
	{
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('new-HPOVLogicalSwitch {0}{1} `' -f $namegroupParam, $managedParam) -isVar $False  -indentlevel 1))
...
Function Import-SANmanager([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ( $san in $List)
	{
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('new-HPOVSANManager {0} `' 	-f $nameParam) -isVar $False  -indentlevel 1))
...
Function Import-StorageSystem([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ( $sts in $List)
	{
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('new-HPOVStorageSystem {0}{1} `' 	-f $nameParam, $authParam) 	-isVar $False  -indentlevel 1))

		if ($family -eq 'StoreServ')
		{
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('{0} | wait-HPOVTaskComplete' 	-f $domainParam) 			-isVar $False  -indentlevel 3))

		}
		else
		{
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('{0} | wait-HPOVTaskComplete'	-f $vipsParam) 				-isVar $False  -indentlevel 3))
		}
		newLine
		...
		if ($storagePool)
		{
			...
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'sts' 						-Value ("get-HPOVStorageSystem | where name -eq  '{0}' " -f $name ) -indentlevel 1 ))	
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'new-HPOVStoragePool -pool $pool -StorageSystem $sts | wait-HPOVTaskComplete'  -isVar $False  -indentlevel 1))
			# Set pool to managed state
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix '$pool | % {get-HPOVStoragePool -name $_ | set-HPOVStoragePool -Managed $true }'  -isVar $False  -indentlevel 1))
...
Function Import-StorageVolumeTemplate([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ( $svt in $List)
	{
		...
		{	
			...
			if ($folder)
			{
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'folder' 	-Value ("(get-HPOVStoragePool -name default).deviceSpecificAttributes.Folders | where name -eq  '{0}' " -f $folder ) -indentlevel 2 ))	
			...
			{
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'performancePolicy' 	-Value ('Show-HPOVStorageSystemPerformancePolicy -InputObject $sts -name "{0}" ' -f $performancePolicy ) -indentlevel 2 ))	
			...
			#---- code here
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('new-HPOVStorageVolumeTemplate {0}{1}{2} `'	-f $nameParam, $capacityParam, $storagePoolParam )	-isVar $False  -indentlevel 2))
...
Function Import-StorageVolume([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ( $vol in $List)
	{
		...
		if ($volumeTemplate)
		{
			...
			#---- code here
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('new-HPOVStorageVolume {0}{1}'	-f $nameParam, $volumeTemplateParam )	-isVar $False  -indentlevel 2))
		...
		else # standalone volume no template
		{
			...
			{	
				...
				#---- code here
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('new-HPOVStorageVolume {0}{1}{2} `'	-f $nameParam, $capacityParam, $storagePoolParam )	-isVar $False  -indentlevel 2))
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('{0}{1} `'									-f $attributes1Param, $attributes2Param) 			-isVar $False  -indentlevel 3))
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('{0} `'										-f $attributes3Param) 								-isVar $False  -indentlevel 3))
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('{0}'										-f $attributes4Param) 								-isVar $False  -indentlevel 3))
...
Function Import-EnclosureGroup([string]$sheetName, [string]$WorkBook, [string]$ps1Files )
{
	...
	foreach ( $eg in $List)
	{
		...
		if ($ligMapping)
		{
			...
			{
				$value 			= "Get-HPOVLogicalInterconnectGroup -name '{0}'" -f $varName
				$variable 		= "lig$i"
		...
		switch ($deploymentMode)
		{
			'External' 
				{
					[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'deployNetwork' -Value ('Get-HPOVNetwork -name "{0}"' -f $deploymentNetwork) -indentlevel 1))
		...
		newLine
		[void]$PSscriptCode.Add(( Generate-PSCustomVarCode -Prefix $( 'New-HPOVEnclosureGroup	-Name $name -enclosureCount $enclosureCount {0}{1} `' -f $v4AddressPoolParam, $v6AddressPoolParam) -isVar $False -indentlevel 1))
	...
	foreach ( $eg in $List)
	{
		...
		if ($ligMapping)
		{
			...
			foreach ($_ligName in $varArray)
			{
				...
				[void]$YMLscriptCode.Add((Generate-ymlTask 	-title $title -isData $False 	-HPOVTask 'oneview_logical_interconnect_group_facts'))
				...
				[void]$YMLscriptCode.Add((Generate-ymlTask 	-title $title -isData $False 	-HPOVTask 'oneview_sas_logical_interconnect_group_facts'))
				[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix name			-value $_ligName									-indentlevel 2 ))
				[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix set_fact 		-isVar $True 										-indentlevel 1 ))
		...
		if ($ipV4Range)
		{
			...
			foreach ($_range in $rangeArr)
			{
				...
				[void]$YMLscriptCode.Add((Generate-ymlTask 	-title $title -isData $False 		-HPOVTask 'oneview_id_pools_ipv4_range_facts'))
		...
		[void]$YMLscriptCode.Add((Generate-ymlTask 	-title $title 	-HPOVTask 'oneview_enclosure_group'))
		[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix 	name						-value $name					-indentlevel $indentDataStart ))
...
Function Import-LogicalEnclosure([string]$sheetName, [string]$WorkBook, [string]$subdir)
{
	...
	foreach ( $le in $List)
	{
		...
		if ( ($enclosureNewName) -and ($enclosureName -ne $enclosureNewName))		# If there are neww names
		{
			...
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'this_enclosure' -Value 'Get-HPOVEnclosure | where serialNumber -Match $serialNumbers[$i]'  -indentlevel 2))
			ifBlock -condition 'if ($this_enclosure)   ' 		-isVar $False -indentlevel 2
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'Set-HPOVEnclosure -inputObject $this_enclosure -Name $enclosureNewNames[$i]' -isVar $False -indentlevel 3))	
		...
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix enclosure -value ("Get-HPOVEnclosure | where serialNumber -match '{0}' " -f $enclosure) -indentlevel 1 ))
		$enclParam 				= ' -Enclosure $enclosure'

		# --- EnclosureGroup
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix enclosureGroup -value ("Get-HPOVEnclosureGroup -name '{0}' " -f $enclosureGroup) -indentlevel 1 ))
		...
		if ($fwBaseline)
		{
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix fwBaseline -value ("Get-HPOVBaseline -SPPname '{0}' " -f $fwBaseline) -indentlevel 1 ))
		...
		newLine
		[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('New-HPOVLogicalEnclosure -Name "{0}" {1}{2} `' 	-f $name, $enclParam, $egParam ) 	-isVar $false -indent 1) )
		...
		{
			newLine
			[void]$PSscriptCode.Add( (Generate-PSCustomVarCode -Prefix 'object' -Value 'get-HPOVLogicalEnclosure | where name -eq $name' -indentlevel 1))
			generate-scopeCode -scopes $scopes -indentlevel 1
	...
	foreach ( $le in $List)
	{
		...
		if ($enclosureSN)
		{
			...
			foreach ($_sn in $snArr)
			{
				...
				[void]$YMLscriptCode.Add((Generate-ymlTask 	-title $title -isData $False 		-HPOVTask 'oneview_enclosure_facts'))
				[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix set_fact 			-isVar $True 							-indentlevel 1 ))
			...
			[void]$YMLscriptCode.Add((Generate-ymlTask 	-title $title -isData $False 		-HPOVTask 'oneview_enclosure_group_facts'))
			...
			if ($fwBaseline)
			{
				...
				[void]$YMLscriptCode.Add((Generate-ymlTask 	-title $title -isData $False 		-HPOVTask 'oneview_firmware_driver_facts'))
			...
			[void]$YMLscriptCode.Add((Generate-ymlTask 	-title $title 	-comment $comment		-HPOVTask 'oneview_logical_enclosure'))
			[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix name				-value $name						-indentlevel $indentDataStart ))
			...
			if ( ($enclosureNewName) -and ($enclosureName -ne $enclosureNewName) ) 
			{
				...
				for ($i=0 ; $i -lt $snArr.Count ; $i++)
				{
					...
					[void]$YMLscriptCode.Add((Generate-ymlTask 	-title $title -iseTag $True			-HPOVTask 'oneview_enclosure'))
			...
			if ($manualAddresses)
			{
				...
				[void]$YMLscriptCode.Add((Generate-ymlTask 			-title $title 				-state reconfigured		-HPOVTask 'oneview_logical_enclosure'))
				[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix name				-value $name						-indentlevel $indentDataStart ))
			...
			if ($fwBaseline)
			{
				...
				[void]$YMLscriptCode.Add((Generate-ymlTask 			-title $title 					-state firmware_updated		-HPOVTask 'oneview_logical_enclosure'))
				[void]$YMLscriptCode.Add((Generate-YMLCustomVarCode -prefix name					-value $name						-indentlevel $indentDataStart ))
...
Function Import-ProfileorTemplate([string]$sheetName, [string]$WorkBook, [string]$ps1Files, [Boolean]$isSpt, [string]$subdir )
{
	...
	foreach ( $prof in $spList)
	{
		...
		if ($isSpt)
		{
			$getCmd 			= 'get-HPOVServerProfileTemplate'
			$newCmd 			= 'new-HPOVServerProfileTemplate'
			$saveCmd 			= 'save-HPOVServerProfileTemplate'
		}
		else 
		{
			$getCmd 			= 'get-HPOVServerProfile'
			$newCmd 			= 'new-HPOVServerProfile'
		...
		if ($template)
		{
			...
			$value 			 = "Get-HPOVServerProfileTemplate -name '{0}'" -f $template
		...
		if ($serverHardware)
		{
			...
			$value 			 = "Get-HPOVServer -name '{0}'" -f $serverHardware
			[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix server -value $value -indentlevel 1 ))
			# -- Add code to power off server
			$value 			= 'Stop-HPOVServer -inputObject $server -force -Confirm:$False| Wait-HPOVTaskComplete	'
	...
	if ($Null -eq $template) 			# Standalone SP or SPT
	{
			...
			if ($manageFirmware)
			{
				...
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix 'fwBaseline' -Value 'Get-HPOVbaseline -SPPname $sppName' -indentlevel 1))
			...
			if ($Null -ne $lsList)
			{
				...
				$value 				= 'Search-HPOVAssociations ENCLOSURE_GROUP_TO_LOGICAL_INTERCONNECT_GROUP -parent $eg' 
				...
				$value 				= 'if ($sasLigUri) { Send-HPOVRequest -uri $sasLigUri } else {$Null}'
				...
				$value 				= '(Search-HPOVAssociations LOGICAL_INTERCONNECT_GROUP_TO_LOGICAL_INTERCONNECT -Parent $sasLig).childUri[0]' 
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix sasUri -value $value -indentlevel 1 ))

				$value 				= 'if ($sasUri) { Send-HPOVRequest -uri $sasUri } else {$Null}'
				...
				foreach ($ls in $lsList)
				{
					...
					if ($mode)
					{
						...
						if ($logicalDrives)
						{
							...
							{	
								...
								[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix $logicalDisk  -value ('New-HPOVServerProfileLogicalDisk {0} ' -f $ldParam)	-indentlevel 1))
						...
						[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix $controller  -value ('New-HPOVServerProfileLogicalDiskController {0}' -f $controllerParam) -indentlevel 1))
						newLine
					...
					else # SasLogicalJBOD
					{
						...
						ifBlock 	-condition 'if ($sasLI)			# If SAS logical Interconnect exists' -indentlevel 1
							$value 					= 'Get-HPOVAvailableDriveType -InputObject $sasLI | where { $_.Capacity -eq $MaxDriveSize -and $_.Type -eq $DriveTechnology -and $_.NumberAvailable -eq $numPhysicalDrives}'
							...
							ifBlock	-condition 'if ($availableDrives)		# if there are such drives in SAS' -indentlevel 2
								...
								{
									$value 				= 'Get-HPOVLogicalJBOD -name "{0}" ' -f $logicalDrives
								}
								else
								{
									$value 				= 'new-HPOVLogicalJBOD -InputObject $sasLI {0} ' -f $JBODParam
								}
								$jbodDisk 			= '$' + "jbod$jbodIndex"
			...
			if ($volList)
			{
				...
				foreach ($_vol in $volList)
				{
					...
					$_volUri 					= '(Get-HPOVStorageVolume -name "{0}" ).uri' -f $_volName

					$_lunType 					= $_vol.volumeLunType
					$_lun						= $_vol.volumeLUN

					$_sts						= $_vol.volumeStorageSystem
					$_stsUri 					= '(Get-HPOVStorageSystem -name "{0}" ).uri' -f $_sts
			...
			if ($manageIlo)
			{
						...
						'LocalAccounts'
							{
								if ($s.username)
								{
									...
									$value 				= 'new-HPOVIloLocalUserAccount	{0}{1} `' 		-f $iloNameParam, $iloDisplayParam 
						...
						'DirectoryGroups'
							{
								if ($s.groupDN)
								{
									...
									[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix $iloDirectoryGroup 	-value ('New-HPOVIloDirectoryGroup{0} `' -f $iloDirGroupParam1) -isVar $False -indentlevel 1))
									foreach ($_param in $_privParamArr)
									{
										[void]$PSscriptCode.Add((Generate-PSCustomVarCode -Prefix ('{0} `'	-f $_param)		-isVar $False	-indentlevel 18))
									}	
				...
				[void]$PSscriptCode.Add((Generate-PSCustomVarCode 	-Prefix 'iloPolicy' -value ('new-HPOVServerProfileIloPolicy	{0} `' -f $iloAdminParam)	-indentlevel 1))
...
Function Import-YMLProfileorTemplate([string]$sheetName, [string]$WorkBook, [string]$YMLFiles, [Boolean]$isSpt, [string]$subdir )
{
	...
	foreach ( $prof in $spList)
	{
		...
		if (($isSp) -and ($null -ne $serverHardware))
		{
			...
			[void]$YMLscriptCode.Add((Generate-ymlTask 		 	-title $title  -HPOVTask 'oneview_server_hardware'))
		...
		[void]$YMLscriptCode.Add((Generate-ymlTask 		 	-title $title -comment $comment -HPOVTask $newFact))
...
{ 
	if  (test-path -path $workBook)
	{
		...
		[void]$allPSscriptCode.Add($CR)
		[void]$allPSscriptCode.Add('#----------------------------------------------')
        [void]$allPSscriptCode.Add('#              TBD -HPOV Appliance configuration		 ')
		[void]$allPSscriptCode.Add('#----------------------------------------------')
		[void]$allPSscriptCode.Add($CR)
